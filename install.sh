...
echo "      .problem/local/ 은 로컬 전용이므로 .gitignore에 추가하세요."
echo "      .problem/frozen-tests.txt 는 감사용 임시 커밋 대상이며 최종 커밋에서 인덱스만 제거되고, 배포 성공 후 로컬에서도 삭제됩니다."
if git check-ignore -q --no-index .problem/frozen-tests.txt 2>/dev/null; then
  echo "  [!!] 기존 .gitignore가 .problem/ 전체를 제외합니다."
  echo "       하네스는 감사 파일을 git add -f로 처리하지만, 가능하면 .problem/ 규칙을 .problem/local/ 규칙으로 좁혀주세요."
fi
