...
동일한 배포 워크플로우를 **Claude Code · Codex · Gemini** 세 하네스에서 공유합니다.
...
| 하네스 | 폴링 모드 | Co-Author 트레일러 |
|--------|-----------|--------------------|
| Claude Code | `run_in_background: true` Bash + `while` 루프 (완료 시 세션 자동 재호출) | `Co-Authored-By: Claude <모델명> <noreply@anthropic.com>` |
| Codex | 포그라운드 블로킹 `while` 루프 (한 셸 호출로 완료까지 대기) | `Co-Authored-By: Codex <noreply@openai.com>` |
| Gemini | 포그라운드 블로킹 `while` 루프 | `Co-Authored-By: Gemini <noreply@google.com>` |
...
- **단독 모드**(폴백): Codex CLI가 없거나 인증되지 않으면 메인 하네스가 테스트와 구현을 모두 작성합니다. 동결은 없지만 "테스트 먼저 → 실패 시 구현만 수정" 원칙은 유지됩니다.
- 테스트가 스펙을 잘못 해석했다고 판단되면 `⚠️ TEST-DISPUTE`로 이의를 제기합니다.

> **Codex 호출 hang 4중 방어**: Codex 테스트 생성 호출이 무한 대기하는 사고를 ① 프롬프트 stdin 리다이렉트(`< "$CODEX_PROMPT"`), ② 출력 파일 리다이렉트(파이프 금지), ③ 프롬프트에서 빌드/테스트 실행 금지, ④ `timeout` 상한의 4중으로 차단합니다. 자세한 원리는 `.claude/commands/problem.md`의 Phase 4를 참고하세요.

> Codex CLI는 **선택 사항**입니다. 미설치 시 자동으로 단독 모드로 진행합니다.

### 병렬 워크트리 실행 & cherry-pick 통합 (`/problem`, Claude Code)

독립적인 작업 그룹은 Agent tool로 병렬 실행(각자 git worktree에서 로컬 커밋)하고, 완료 후 각 에이전트가 보고한 커밋 SHA를 메인 세션이 cherry-pick으로 통합합니다.

### 메인 세션 직접 배포 파이프라인

배포(`pipeline.md`)는 서브에이전트에 위임하지 않고 **메인 세션(또는 단일 실행 흐름)이 직접** 수행하며, CI/CD·PR 체크 대기는 하네스별 폴링 모드로 처리합니다.
...
**방법 2: 복잡한 문제** — 먼저 대화하며 문제와 해결책을 구체화한 뒤 `/problem` 실행

```
# 1. 먼저 대화로 문제 파악
> 로그인 후 토큰 갱신이 안 되는 것 같은데 원인이 뭘까?
> (대화하며 원인 분석 및 해결 방향 정리)
...
1. **해결책 선택** (Phase 1) — 제시된 해결책 중 적용할 번호를 선택
2. **최종 확인** (Phase 5) — 모든 코드 수정 및 테스트 완료 후 결과 확인 → 추가 수정이 필요하면 요청, 문제없으면 OK로 다음 단계(커밋·PR·배포) 진행

아래 2곳은 **조건부**로만 개입이 발생합니다:

- **Codex 인증** (Phase 4) — Codex CLI가 설치돼 있으나 인증이 필요한 경우에만, Codex 모드 진행 / 단독 모드 중 선택을 요청 (미설치면 개입 없이 자동으로 단독 모드)
...
**이유**: AI가 코드를 작성할 때 가장 큰 위험은 "돌아가는 것 같지만 실제로는 스펙과 다른 코드"입니다. **테스트 작성자(Codex)와 구현자(메인 하네스)를 분리**하면 테스트가 진짜 독립 명세(spec)가 되어, 구현이 스펙에 맞는지 자동 검증됩니다. 테스트를 동결해 "실패 시 테스트가 아닌 구현을 수정"하는 규칙을 훅+감사 두 층으로 강제함으로써 이 명세 역할을 보호합니다. (Codex를 못 쓰는 환경에서는 단독 모드로 폴백하되 Test-First 원칙은 유지합니다.)
...
배포 파이프라인(커밋 → PR → CI → 머지 → CD)을 서브에이전트에 위임하지 않고 **메인 세션이 직접** 수행하며, CI/CD·PR 체크 대기는 `run_in_background` Bash 폴링(Codex/Gemini는 포그라운드 블로킹 루프)으로 처리합니다.

**이유**: 파이프라인은 "액션 → 폴링 → 다음 액션"의 반복인데, 서브에이전트 안에서 백그라운드 폴링을 돌리면 완료 후 세션 재개가 안 돼 대기 지점에서 멈추는 문제가 실측되었습니다. 메인 세션의 `run_in_background` Bash는 완료 시 세션을 확실히 재호출하므로, 긴 대기가 섞인 파이프라인을 안정적으로 이어갈 수 있습니다. 폴링 루프는 GitHub API 조회 실패를 pending으로 삼키지 않고 3회 연속 실패 시 `API_ERROR`로 중단하여, 네트워크 장애를 CI 대기로 오판하지 않습니다.
...
**이유**: AI 에이전트가 `git add -A`를 실행하면 의도치 않게 시크릿이 커밋될 수 있고, RED 테스트를 통과시키지 못할 때 "테스트를 고쳐버리는" 우회 유혹이 생깁니다. 두 훅은 **자동화의 편의성을 유지하면서 보안 사고와 명세 훼손을 예방**합니다. (동결 훅은 예방, Phase 5의 `git diff` 감사는 탐지 — 두 층으로 Bash 우회 편집까지 방어합니다.) 훅이 없는 Codex/Gemini에서는 동일 규칙을 명세로 LLM이 직접 준수합니다.
