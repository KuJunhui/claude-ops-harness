...
2. 전체 빌드 검증
3. **병렬 작업이 있었던 경우** 또는 **`[비API]` 작업에서 기존 테스트를 수정한 경우** 전체 테스트를 실행한다 (워크트리 통합 후 호환성 검증 및 테스트 수정의 부작용 확인 목적). 해당하지 않으면 Phase 4에서 이미 검증 완료이므로 스킵한다.
