...
|------|-------|---------------|-------------|
| `[FEAT]` | `FEAT` | `feat/` | 프로젝트 이슈 템플릿 |
| `[FIX]` | `FIX` | `fix/` | 프로젝트 이슈 템플릿 |
| `[BUG]` | `BUG` | `bug/` | 프로젝트 이슈 템플릿 |
| `[CHORE]` | `CHORE` | `chore/` | 프로젝트 이슈 템플릿 |
| `[REFACT]` | `REFACT` | `refact/` | 프로젝트 이슈 템플릿 |
| `[DOCS]` | `DOC` | `docs/` | 프로젝트 이슈 템플릿 |

2. `gh issue create` — 제목: `[TYPE] 설명` (선택된 해결책들을 요약), 본문: 이슈 템플릿의 구조를 따르되 Phase 2의 실행 계획 기반으로 내용 채움
...
3. 모든 변경 파일 재검토: 문법/컴파일 에러, import 누락, 호환성, 보안 취약점
...
3. **병렬 작업이 있었던 경우에만** `./gradlew test` 전체 테스트 실행 (워크트리 통합 후 호환성 검증 목적). 순차 작업만이었으면 Phase 4에서 이미 검증 완료이므로 스킵한다.
...
4. PR 템플릿을 읽어서 placeholder를 채운 뒤 → `gh pr create` — base: `dev`, head: `"<브랜치명>"`
   - 제목: `[TYPE/#이슈번호] 설명`
   - 출력 URL 끝의 숫자가 PR 번호 → 이후 단계에서 `<dev PR 번호>`로 사용
5. 머지 가능 여부 확인 (30초 간격, 최대 5분):
   ```bash
   SECONDS=0
   until [ "$(gh pr view <dev PR 번호> --json mergeable --jq '.mergeable')" != "UNKNOWN" ]; do
     if [ $SECONDS -ge 300 ]; then echo "MERGEABLE still UNKNOWN after timeout"; exit 1; fi
     sleep 30
   done
   MERGEABLE=$(gh pr view <dev PR 번호> --json mergeable --jq '.mergeable')
   ```
   - `MERGEABLE` → `gh pr merge <dev PR 번호> --squash` (--auto 사용 금지)
...
1. dev push → CI 워크플로우 자동 트리거
2. CI 폴링 (60초 간격, 최대 15분):
   ```bash
   SECONDS=0
   until [ "$(gh run list --branch dev --workflow <ci-workflow>.yml --commit $MERGE_SHA --limit 1 --json status --jq '.[0].status')" = "completed" ]; do
     if [ $SECONDS -ge 900 ]; then echo "CI TIMEOUT"; exit 1; fi
     sleep 60
   done
   gh run list --branch dev --workflow <ci-workflow>.yml --commit $MERGE_SHA --limit 1 --json conclusion --jq '.[0].conclusion'
   ```
   - timeout 시 에러 내용을 반환하고 **서브에이전트 종료**
3. CI 결과:
   - 성공:
     - `<배포 이슈 번호>`가 전달된 경우 → 해당 번호를 재사용
     - 전달되지 않은 경우 → 배포 이슈 생성: `gh issue create --title "[CHORE] 배포" --label "CHORE"` (본문: 이슈 템플릿 구조를 따르되 PR 내용 요약으로 채움) → 출력 URL 끝의 숫자를 이후 단계에서 `<배포 이슈 번호>`로 사용
   - 실패 → run-id 확인 후 `gh run view <run-id> --log-failed` 로그 요약을 반환하고 **서브에이전트 종료**
4. 배포 PR 생성:
   - `<배포 PR 번호>`가 전달된 경우 → 해당 번호를 재사용
   - 전달되지 않은 경우 → PR 템플릿을 읽어서 placeholder를 채운 뒤 → `gh pr create --base main --head dev --title "[CHORE/#배포이슈번호] 배포"` (본문에 `close #<배포이슈번호>` 포함)
5. 배포 PR 체크 폴링 (60초 간격, 최대 15분) — 필요한 체크가 모두 통과할 때까지 대기:
   ```bash
   SECONDS=0
   until [ "$(gh pr checks <배포 PR 번호> --json name,state --jq '<프로젝트에 맞는 체크 조건>')" = "true" ]; do
...
1. main push → CD 워크플로우 자동 트리거
2. 머지 커밋 SHA 확인: `CD_SHA=$(gh pr view <배포PR번호> --json mergeCommit --jq '.mergeCommit.oid')`
3. CD 폴링 (60초 간격, 최대 20분):
   ```bash
   SECONDS=0
   until [ "$(gh run list --branch main --workflow <cd-workflow>.yml --commit $CD_SHA --limit 1 --json status --jq '.[0].status')" = "completed" ]; do
     if [ $SECONDS -ge 1200 ]; then echo "CD TIMEOUT"; exit 1; fi
     sleep 60
   done
   gh run list --branch main --workflow <cd-workflow>.yml --commit $CD_SHA --limit 1 --json conclusion --jq '.[0].conclusion'
   ```
   - timeout 시 에러 내용을 반환하고 **서브에이전트 종료**
4. CD 결과:
   - 성공 → 아래 cleanup을 순서대로 시도하되, **각 명령이 실패해도 중단하지 않고 계속 진행**한다 (실패 내역만 기록):
     ...
     2. `git branch -D "<브랜치명>"`
     3. `git push origin --delete "<브랜치명>"` (이미 삭제된 경우 무시)
     4. 사용된 워크트리 정리: `.claude/worktrees/` 내 에이전트 디렉토리를 `git worktree remove <path> --force`로 제거 후 `git worktree prune` 실행
   - 실패 → `gh run view <run-id> --log-failed`로 워크플로우 로그 요약과 **배포 이슈 번호**를 반환하고 **서브에이전트 종료**
...
2. 아래 재시작 매핑에 따라 재시작 Phase를 결정한다. **코드 수정 또는 dev push가 수반되면 반드시 Phase 7a (CI + PR 체크 대기)를 다시 거쳐야 한다 — CI 검증 없는 배포는 금지한다.**
   ...
   | Phase 7a-2 | CI 실패 | 코드 수정 → **Phase 6**부터 | 새 커밋 필요, 새 dev PR 생성 |
...
4. 임시 브랜치 push 및 배포 PR 생성:
   ...
   ```
5. **CI 체크 스킵 판단**: CI가 dev 브랜치에서만 트리거되는 프로젝트의 경우, main 기반 임시 브랜치에서는 CI 체크가 실행되지 않는다. 동일 코드가 dev CI를 이미 통과한 상태이므로, **체크 대기 없이 즉시 squash merge**한다. CI가 모든 브랜치에서 트리거되는 프로젝트라면 체크를 대기한다.
...
- **`dev` 브랜치에서 `git pull origin main`, `git merge main`, `git merge origin/main` 등 main을 dev로 merge하는 행위는 절대 금지한다.** 이 워크플로우는 dev → main 단방향 플로우이므로 main에 dev에 없는 커밋이 존재하지 않는다. squash merge 환경에서 main을 dev로 끌어오면 커밋 중복, 불필요한 merge 커밋, 충돌이 발생한다. 어떤 Phase에서도 자의적으로 브랜치 동기화를 시도하지 않는다.
