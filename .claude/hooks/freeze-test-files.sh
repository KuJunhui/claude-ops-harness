#!/usr/bin/env bash
#
# Claude Code PreToolUse hook: Codex가 작성한 동결 테스트 파일의 Edit/Write 차단
#
  ...
  if [ "$REL_PATH" = "$frozen" ] || [ "$FILE_PATH" = "$frozen" ]; then
    jq -n --arg f "$REL_PATH" '{
      hookSpecificOutput: {
        ...
        permissionDecisionReason: ("🧊 TEST-FREEZE: " + $f + " — Codex가 작성한 동결 테스트입니다. 구현 코드를 수정하여 테스트를 통과시키세요. 테스트가 스펙을 잘못 해석했다면 ⚠️ TEST-DISPUTE로 이의를 제기하세요.")
      }
    }'
    exit 0
  fi
